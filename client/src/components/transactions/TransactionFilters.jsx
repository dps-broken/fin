// client/src/components/transactions/TransactionFilters.jsx
import React, { useState, useEffect } from 'react'; // React first
import { AnimatePresence, motion } from 'framer-motion'; // External libs
...
import {
  ...
  ArrowDownIcon,
  XMarkIcon,
} from '@heroicons/react/24/outline'; // Grouped imports
import Select from '../common/Select'; // Local imports
...
function TransactionFilters({
  ...
  const handleSubmit = (e) => {
    ...
    const filtersToApply = {
      ...
      minAmount: localFilters.minAmount
        ? parseFloat(localFilters.minAmount)
        : '',
      maxAmount: localFilters.maxAmount
        ? parseFloat(localFilters.maxAmount)
        : '',
    };
    Object.keys(filtersToApply).forEach((key) => { // Prettier might format (key)
      if (
        filtersToApply[key] === '' ||
        filtersToApply[key] === null ||
        Number.isNaN(filtersToApply[key])
      ) {
  ...
  const handleSortFieldChange = (selectedOption) => {
    const newSortBy = selectedOption ? selectedOption.value : 'date';
    setSortBy(newSortBy);
  ...
  const categoryOptions = categories
    .filter((cat) => !localFilters.type || cat.type === localFilters.type)
