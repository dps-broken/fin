// client/src/components/budgets/BudgetItem.jsx
import React from 'react'; // React first
import { motion } from 'framer-motion'; // External libraries next
...
import {
  ...
  TagIcon,
  CalendarDaysIcon,
} from '@heroicons/react/24/outline'; // Grouped imports, Prettier might wrap this
import Card, { CardBody, CardFooter } from '../common/Card'; // Local imports
import BudgetProgress from './BudgetProgress';

// Changed to function declaration as per common ESLint (Airbnb) preference
function BudgetItem({ budget, onEdit, onDelete }) {
  const {
    ...
    endDate,
    totalSpent,
    // progress, // Not directly used if totalSpent and amount are available for BudgetProgress
  } = budget;

  const getPeriodText = () => {
    if (period === 'monthly') {
      return `Monthly (from ${format(parseISO(startDate), 'MMM d, yyyy')})`;
    }
    if (period === 'yearly') {
      return `Yearly (from ${format(parseISO(startDate), 'MMM d, yyyy')})`;
    }
      ...
      <Card className="flex flex-col h-full" hoverEffect>
        <CardBody className="flex-grow">
          <div className="flex justify-between items-start mb-2">
            <h3
              className="text-lg font-semibold text-primary-light dark:text-primary-dark truncate"
              title={name}
            >
              {name}
            </h3>
            <div className="flex space-x-1">
              <button
                type="button" // Added type="button" for accessibility/semantics
        ...
        <CardFooter className="bg-gray-50 dark:bg-slate-700/50">
          ...
          <div className="mt-2 flex justify-between text-xs text-text-muted-light dark:text-text-muted-dark">
            ...
            <span>
              Remaining: ₹{(amount - (totalSpent || 0)).toFixed(2)}
            </span>
          </div>
        </CardFooter>
      </Card>
